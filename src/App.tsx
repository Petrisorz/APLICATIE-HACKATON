          ...
          <div className="modal-content">
            ...
            {glicemie > 140 && (
              <div style={{ background: '#ffcdd2', padding: '12px', borderRadius: '15px', textAlign: 'left', marginBottom: '15px', border: '1px solid #d32f2f' }}>
                ...
                <p style={{ fontSize: '12px', margin: '5px 0 0 0', color: '#b71c1c' }}>Ai depășit limita. Evită complet produsele bogate în zaharuri din frigider și hidratează-te!</p>
              </div>
            )}

            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setGlicemie(prev => prev + 60)} style={{ flex: 1, background: '#ef5350', color: 'white', border: 'none', padding: '12px', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}>⬆️ RAISE</button>
              <button onClick={() => setGlicemie(90)} style={{ background: '#eee', border: 'none', padding: '12px', borderRadius: '12px', cursor: 'pointer' }}>Reset</button>
              <button onClick={() => setGlicemie(prev => prev - 40)} style={{ flex: 1, background: '#42a5f5', color: 'white', border: 'none', padding: '12px', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}>⬇️ REDUCE</button>
            </div>
          </div>
        </div>
      )}
                ...
                return (
                  <div key={item.id} className="card" style={{ background: hasAlergiePericol ? '#fff5f5' : 'white', border: hasAlergiePericol ? '2px solid #d32f2f' : '1px solid #eee', padding: '15px' }}>
                    ...
                    {item.alergeniDetectati.length > 0 && (
                      <div style={{ display: 'inline-block', border: '1px solid #ef5350', color: '#c62828', padding: '3px 6px', borderRadius: '6px', fontSize: '10px', marginTop: '10px', fontWeight: 'bold' }}>
                        Conține: {item.alergeniDetectati.join(', ')}
                      </div>
                    )}

                    {hasAlergiePericol && (
                      <div style={{ background: '#d32f2f', color: 'white', padding: '8px', borderRadius: '8px', marginTop: '10px', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <AlertTriangle size={14} /> <strong>PERICOL: {pericolCurent.join(', ')}</strong>
                      </div>
                    )}

                    <button onClick={() => toggleExpand(item.id)} style={{ width: '100%', background: '#f8f9fa', border: '1px solid #eee', borderRadius: '8px', color: '#666', padding: '6px', marginTop: '12px', display: 'flex', justifyContent: 'center', cursor: 'pointer' }}>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: '1px solid #eee', paddingTop: '12px', marginTop: '10px' }}>
                        <div style={{ marginBottom: '12px', background: '#f9f9f9', padding: '12px', borderRadius: '10px' }}>
                           <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#555', marginBottom: '8px', fontWeight: 'bold' }}>
                             <span>MAI AI: {item.procentRamas}%</span>
                             <span>Pachet ({item.gramajTotal}g)</span>
                           </div>
            ...
            <div style={{ textAlign: 'center', background: 'white', padding: '20px', borderRadius: '20px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
              ...
              <div style={{ position: 'relative', width: '150px', height: '150px', margin: '15px auto' }}>
                <svg width="150" height="150">
                  ...
                  <circle cx="75" cy="75" r="65" stroke={dietKcalConsumed > safeDietMax ? "#d32f2f" : "#1b5e20"} strokeWidth="12" fill="none"
                    strokeDasharray={408} strokeDashoffset={408 - (Math.min(dietKcalConsumed / safeDietMax, 1) * 408)} 
                    strokeLinecap="round" transform="rotate(-90 75 75)" style={{ transition: 'stroke-dashoffset 0.6s ease' }} 
                  />
                </svg>
              ...
              <p style={{ color: dietKcalConsumed > safeDietMax ? '#d32f2f' : '#1b5e20', fontWeight: 'bold', marginTop: '10px', fontSize: '14px' }}>
                {safeDietMax - dietKcalConsumed > 0 ? `${safeDietMax - dietKcalConsumed} kcal left` : `Ai depășit cu ${dietKcalConsumed - safeDietMax} kcal!`}
              </p>
                ...
                return (
                  <div key={item.id} className="card" style={{ padding: '15px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                      ...
                      <div style={{ fontSize: '11px', color: '#ef5350', fontWeight: 'bold', background: '#ffebee', padding: '4px 6px', borderRadius: '6px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                      ...
                      <button onClick={() => consumaProdus(item)} style={{ background: '#e8f5e9', color: '#1b5e20', border: '2px solid #1b5e20', padding: '10px 15px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}>
                        CONSUMĂ
                      </button>
                    </div>
                  </div>
